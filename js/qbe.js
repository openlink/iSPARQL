...
function init_qbe () {
    ...
    OAT.SVGSparqlNode.prototype.setValueByDrop = function(val, t, x, y) {
        if (t != 'class') {
            ...
            for (var i = 0; i < this.edges.length; i++) {
                /* try to find existing rdf:type node */
                var e = this.edges[i];
                var l = e.getLabel(1);
                if (e.node1 == this && (l == "rdf:type" || l == "a" || l == "type")) {
                    goodNode = e.node2;
                }
                if (e.node2 == this && (l == "rdf:type" || l == "a" || l == "type")) {
                    goodNode = this;
                }
            }
            if (!goodNode) {
    ...
    OAT.SVGSparqlNode.prototype.MySetLabel = function(which, newLabel) {
        if (which == 1) {
            ...
            if (count == 1) {
                this.svgsparql.qbe.Schemas.DeleteNode(old);
            }
...
iSPARQL.QBE = function(def_obj) {
    ...
    var restrictionFunction = function(new_width, new_height) {
        return new_width < 600;
    }
    ...
    self.orderby_grid.createHeader([{
        value: 'order by',
        sortable: 0,
        draggable: 0,
        resizable: 0
    ...
    this.addOrderBy = function(obj, addmode) {
        ...
        if (!obj.orderby_cell) {
            ...
            var orderby_cell = self.orderby_grid.appendHeader({
                value: label,
                sortable: 1,
                draggable: 1,
                resizable: 0
    ...
    this.putPrefix = function(str) {
        /* replace first part of URI with prefix, if applicable */
        var tmp = '';
        if ((tmp = str.match(/^<(.*)>$/))) {
            /* if is an <URI> */
            var uri = tmp[1];
            for (var i = 0; i < self.prefixes.length; i++) {
                ...
                if (uri.substring(0, prefix.uri.length) == prefix.uri && /* if prefix.uri is left substring of uri */
                    !uri.substring(prefix.uri.length, uri.length).match(/\//)) {
                    /* remaining part doesn't contain slash */
    ...
    this.getPrefixParts = function(str) {
        /* return firstpart, secondpart, prefix (if found) */
        var s = str;
        if (!s) {
            return;
        }
    ...
    self.Schemas = {
        ...
        MergeSchema: function(parent, schema, graphSchema, label, bound) {
            /* insert a new prefix into schemas */
            for (var i = 0; i < parent.children.length; i++) {
                if (parent.children[i].schema == schema) {
        ...
        Remove: function(schema) {
            ...
            if (index != -1) {
                self.Schemas.Imported.splice(index, 1);
            }
        },
        ...
        InsertNode: function(parent, uri_, type, label, comment) {
            ...
            if (type == 'class') {
                ...
                leaf.setImage('relation-icon-16');
            }
        },
        DeleteNode: function(uri) {
            /* only from "Bound" subtree */
            ...
            if (!containerNode.children.length) {
                schemaNode.deleteChild(containerNode);
            }
            if (!schemaNode.children.length) {
                root.deleteChild(schemaNode);
            }
        },
        Update: function(node) {
            /* get Classes and Properties for a prefix */
            if (node.children.length > 0) {
                return;
            } /* nothing when already fetched */
                    ...
                    for (var i = 0; i < objs.length; i++) {
                        ...
                        switch (obj.type.value) {
                            ...
                            case "http://www.w3.org/2002/07/owl#InverseFunctionalProperty":
                                if (self.Schemas.IsAttribute(obj)) {
                                    attrs.push(obj);
                                } else {
                                    rels.push(obj);
                                }
            ...
            var params = {
                ...
                query: 'define get:soft "replacing" \n' +
        ...
        Refresh: function(force) {
            ...
            if (self.Schemas.Unbound.state == 0 && !force) {
                return;
            }
    ...
    OAT.MSG.attach(self.Schemas.Tree, 'TREE_EXPAND', function(sender, msgcode, node) {
        ...
        if (node == self.Schemas.Unbound) {
            self.Schemas.Refresh(true);
        }
    });
    ...
    this.dataset_win.onclose = function() {
        OAT.Dom.hide(self.dataset_win.container);
    }
    OAT.Dom.hide(self.dataset_win.dom.container);
    ...
    this.addDataSource = function(val, type) {
        if (!val) {
            alert('Empty Information Source!');
            return false;
        }
        ...
        var cell_cb = OAT.Dom.create("td");
        cell_cb.innerHTML = '<select id="ds_type_' + self.dataSourceNum + '"><option value="F">From</option><option value="N"' + ((type == 'N') ? ' selected="selected"' :
            '') + '>Named</option></select>';
        row.appendChild(cell_cb);
    ...
    var fileRef = function() {
        ...
        if (name.slice(name.length - ext.length - 1).toLowerCase() != "." + ext) {
            name += "." + ext;
        }
    ...
    this.RunQuery = function() {
        ...
        if (qe.detectLocationMacros(p.query)) {
            ...
            var locUI = new iSPARQL.locationAcquireUI({
                useCB: qe.executeWithLocation,
                cancelCB: false,
                cbParm: p,
    ...
    this.QueryGenerate = function() {
        ...
        for (var j = 0; j < used_prefixes.length; j = j + 2) {
            sq.prefixes.push({
                "label": used_prefixes[j],
                "uri": used_prefixes[j + 1]
            });
        }
    ...
    if (window.__inherited) {
        ...
        if (self.svgsparql) {
            self.loadFromString(self.defaults.query);
            self.svgsparql.reposition();
        }
