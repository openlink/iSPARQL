...
window.defaultPrefixes = [{
    ...
    {
        "label": 'ibis',
        "uri": 'http://purl.org/ibis#',
        "hidden": 1
    },
    {
        "label": 'conf',
        "uri": 'http://www.mindswap.org/~golbeck/web/www04photo.owl#'
    },
    {
        "label": 'scot',
        "uri": 'http://scot-project.org/scot/ns'
    },
    {
        "label": 'ical',
    ...
    {
        "label": 'annotation',
        "uri": 'http://www.w3.org/2000/10/annotation-ns#'
    },
    {
        "label": 'rdfs',
    ...
    {
        "label": 'skos',
        "uri": 'http://www.w3.org/2008/05/skos#'
    },
    {
        "label": 'vs',
    ...
    {
        "label": 'opo',
        "uri": 'http://ggg.milanstankovic.org/opo/ns/'
    },
    {
        "label": 'nco',
        "uri": 'http://www.semanticdesktop.org/ontologies/nco/'
    },
    {
        "label": 'lsdis',
        "uri": 'http://lsdis.cs.uga.edu/projects/meteor-s/wsdl-s/ontologies/LSDIS_FInance.owl'
    },
    {
        "label": 'nao',
        "uri": 'http://www.semanticdesktop.org/ontologies/nao/'
    },
    {
        "label": 'cohere',
        "uri": 'http://cohere.open.ac.uk/ontology/cohere.owl#'
    },
    {
        "label": 'nfo',
    ...
    {
        "label": 'nmo',
        "uri": 'http://www.semanticdesktop.org/ontologies/nmo/'
    },
    {
        "label": 'nie',
        "uri": 'http://www.semanticdesktop.org/ontologies/nie/'
    },
    {
        "label": 'nid3',
        "uri": 'http://www.semanticdesktop.org/ontologies/nid3/'
    },
    {
        "label": 'kuaba',
        "uri": 'http://www.tecweb.inf.puc-rio.br/ontologies/kuaba/'
    },
    {
        "label": 'wot',
...
var QueryExec = function(optObj) {
    ...
    this.queryOptions = {
        ...
        query: false,
        backupQuery: false,
        /* to be executed if original fails */
        defaultGraph: false,
        namedGraphs: [],
        sponge: false,
        maxrows: false,
    ...
    this.makeAnchorPrefs = function() {
        var anchor_pref_c = OAT.Dom.create("div", {
            className: "anchor_pref_c"
        });
        var anchor_label = OAT.Dom.create("label", {
            htmlFor: "anchor_pref_sel"
        });
        anchor_label.innerHTML = "Dereferencing:";
        anchor_pref_sel = OAT.Dom.create("select", {
            id: "anchor_pref_sel"
        });
        ...
        var uri_fmt_c = OAT.Dom.create("span", {
            id: "uri_fmt_ctr"
        });
        var uri_fmt_label = OAT.Dom.create("label", {
            htmlFor: "uri_fmt_ckb"
        });
        uri_fmt_label.innerHTML = "Show Raw URIs";
        uri_fmt_ckb = OAT.Dom.create("input", {
            id: "uri_fmt_ckb"
        });
    ...
    this.init = function() {
        ...
        this.dom.request = OAT.Dom.create("div", {
            className: 'ep_request'
        });
        this.dom.response = OAT.Dom.create("pre", {
            className: 'ep_response'
        });
        ...
        self.dom.tab = OAT.Dom.create("div", {
            className: 'res_tab_ctr',
            id: 'res_tab_ctr'
        });
        self.dom.ul = OAT.Dom.create("ul", {}, "tabres");
        self.tab = new OAT.Tab(self.dom.tab, {
            dockMode: true,
            dockElement: self.dom.ul
        });
        ...
        self.load_notify = new OAT.Notify(false, {
            notifytype: 2
        });
        self.error_notify = new OAT.Notify(false, {
            notifytype: 2
        });
        ...
        OAT.MSG.attach('*',
            ...
            function() {
                ...
                self.load_notify.send(msg, {
                    image: "images/notify-throbber.gif",
                    timeout: false
                });
            });
    ...
    this.buildRequest = function(opts) {
        ...
        if (opts.timeout) {
            paramsObj["timeout"] = opts.timeout;
        }
        if (opts.sponge && self.options.virtuoso) {
            paramsObj["should-sponge"] = opts.sponge;
        }
    ...
    this.addResponse = function(request, opts, wasError, data) {
        /* Cache and visualize */
        if (OAT.AnchorData.window) {
    ...
    this.draw = function(refresh) {
        ...
        item.dom.result_opts_c = OAT.Dom.create("div", {
            className: "result_opts_c"
        });
        item.dom.plnk_c = OAT.Dom.create("div", {
            className: "result_plnk_c"
        });

        item.dom.deref_prefs = self.makeAnchorPrefs();

        var clrfix = OAT.Dom.create("div", {
            className: "clearfix"
        })

        if (!refresh) {
            item.dom.query_c = OAT.Dom.create("div", {
                className: "query_c"
            });
            item.dom.result_c = OAT.Dom.create("div", {
                className: "result_c"
            });
            item.dom.request_c = OAT.Dom.create("div", {
                className: "request_c"
            });
            item.dom.response_c = OAT.Dom.create("div", {
                className: "response_c"
            });
            ...
            if (self.options.executeCallback) {
                self.options.executeCallback(item);
            }
            ...
            for (var i = 0; i < parts.length; i++) {
                req.innerHTML += OAT.Xml.escape(parts[i]) + "\n";
            }
        ...
        if (wasError) {
            ...
            if (item.resType == iSPARQL.ResultType.GRAPH) { // Use RDFMini to show Graphs
                ...
                self.miniplnk = OAT.Dom.create("div", {
                    id: "rdf_plink_c"
                });
                ...
                var mini_c = OAT.Dom.create("div", {
                    className: "rdf_mini_c"
                });

                if (item.store.getTripleCount() > 0) {
                    item.mini = new OAT.RDFMini(mini_c, {
                        tabs: tabs,
                        showSearch: false,
                        store: item.store,
                        defaultTab: 0,
                    ...
                    self.makeMiniRDFPlinkURI(false, false, {
                        tabIndex: lastIndex
                    });
    ...
    this.execute = function(optObj) {
        ...
        for (var p in self.queryOptions) {
            opts[p] = self.queryOptions[p];
        } /* copy of defaults */
        for (var p in optObj) {
            opts[p] = optObj[p];
        }
        ...
        var o = {
            ...
            onerror: onerror,
            headers: {
                Accept: "application/rdf+xml; q=0.1, application/sparql-results+xml; q=0.9"
            }
        };

        if (!opts.endpoint) {
            opts.endpoint = '/sparql';
        }
