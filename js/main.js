...
iSPARQL.QueryExec = function(optObj) {
    ...
    this.go = function(optObj) {
        ...
        if (opts.query in self.cache) {
            if (opts.callback) {
                opts.callback(self.cache[opts.query]);
            }
            return;
        }
        ...
        if (opts.defaultGraph && !opts.query.match(/from *</i)) {
            req["default-graph-uri"] = opts.defaultGraph;
        }
        if (opts.maxrows) {
            req["maxrows"] = opts.maxrows;
        }
        if (opts.sponge && self.options.virtuoso) {
            req["should-sponge"] = opts.sponge;
        }
...
OAT.Observer = {
    add: function(observers, _obj, fun) {
        if (!OAT.Observer.find(observers, _obj, fun)) observers.push({
            o: _obj,
            fun: fun
        });
    },
...
iSPARQL.EndpointOptsUI = function(optsObj, toggler, indicator, container) {
    ...
    this._init = function() {
        self.epCombo = new OAT.Combolist(iSPARQL.Defaults.endpoints,
            "/sparql", {
                name: "service",
                onchange: self.endpointChangeCB
            });
    ...
    this.setGrabVarCtl = function(callerObj, val) {
        return 0;
    }
...
iSPARQL.AuthUI = function(conn) {
    ...
    this.dlg = new OAT.Dialog("Endpoint User Login", "auth_dlg", {
        width: 400,
        modal: 0,
        buttons: 0
    });
...
iSPARQL.Common = {
    ...
    initUI: function() {
        ...
        iSPARQL.dialogs.about = new OAT.Dialog("About iSPARQL", "about_dlg", {
            width: 400,
            modal: 0,
            buttons: 0
        });
        ...
        iSPARQL.dialogs.help = new OAT.Dialog("iSPARQL Help", "help_dlg", {
            width: 400,
            modal: 0,
            buttons: 0
        });
        ...
        iSPARQL.dialogs.prefs = new OAT.Dialog("iSPARQL Preferences", "prefs_dlg", {
            width: 600,
            modal: 1,
            def_layout: false,
            className: "prefs_dlg"
        ...
        OAT.MSG.attach("*", "LOCATION_ACQUIRED",
            function(m, s, l) {
                $("ft_loc").innerHTML = "lat:" + l.getLat() + " lon:" + l.getLon();
            });
        ...
        tab = new OAT.Tab("main_col", { //dockMode:false,
            //dockElement:"tabs",
        ...
        OAT.Event.attach("default-graph-uri",
            "keyup",
            function() {
                iSPARQL.Common.setDefaultGraph($v("default-graph-uri"));
            });
    ...
    initQE: function() {
        ...
        window.qe = new QueryExec({
            div: "page_results",
            executeCallback: execCB
        });
    },
    ...
    handlePageParams: function() {
        ...
        if (p['default-graph-uri']) {
            iSPARQL.Settings.graph = p['default-graph-uri'];
            qp = true;
        }
        if (p['defaultGraph']) {
            iSPARQL.Settings.graph = p['defaultGraph'];
            qp = true;
        }
        if (p['query']) {
        ...
        if (p['sponge']) {
            iSPARQL.Settings.sponge = p['sponge'];
            qp = true;
        }
        if (p['should_sponge']) {
            iSPARQL.Settings.sponge = p['should_sponge'];
            qp = true;
        }
        if (p['view']) {
        ...
        if (p['endpoint']) {
            iSPARQL.Settings.endpoint = p['endpoint'];
            qp = true;
        }
        if (p['resultview']) {
            iSPARQL.Settings.resultView = p['resultview'];
            qp = true;
        }
