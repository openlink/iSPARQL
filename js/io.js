...
iSPARQL.IO = {
    ...
    serializeRq: function(dataObj) {
        ...
        if (dataObj.defaultGraph) {
            str += '# {graph} {' + dataObj.defaultGraph + '}\n';
        }
    ...
    serializeLdr: function(dataObj) {
        ...
        if (dataObj.defaultGraph) {
            addNode(page, iNS, "graph", dataObj.defaultGraph);
        }
        if (dataObj.useProxy) {
            addNode(page, iNS, "proxy", dataObj.useProxy);
        }
        ...
        if (dataObj.canvas) {
            addNode(isparql, iNS, "canvas", dataObj.canvas);
        }
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (p) {
                ...
                for (var j = 0; j < dataObj.pragmas.length; j++) {
                    if (dataObj.pragmas[j][0] == p[0]) {
                        index = j;
                        break;
                    }
                }
                if (index == -1) {
                    dataObj.pragmas.push(p);
                } else {
                    dataObj.pragmas[index][1].push(p[1]);
                }
                continue;
            }
    ...
    unserializeXml: function(str) {
        ...
        if (q) {
            dataObj.query = OAT.Dom.fromSafeXML(q.textContent) || "";
        }
        if (r) {
            dataObj.defaultGraph = r.getAttribute('sql:default-graph-uri') || false;
        }
    ...
    unserializeLdr: function(str) {
        ...
        for (var i = 0; i < pragmas.length; i++) {
            ...
            if (index == -1) {
                dataObj.pragmas.push([p, v]);
            } else {
                for (var j = 0; j < v.length; j++) {
                    dataObj.pragmas[index][1].push(v[j]);
                }
            }
        }
